...
(function () {
    ...
    const observer = new MutationObserver((mutations, obs) => {
        for (const { addedNodes } of mutations) {
            const script = Array.from(addedNodes).find(n => n.nodeName === 'SCRIPT' && n.id === 'script-sugarcube');
            if (script) {
                obs.disconnect();
                const parent = script.parentNode;
                script.remove();
                initializeModSystem(script, parent);
                return;
            }
        }
    });
    ...
    async function initializeModSystem(sugarcubeScript, scriptParent) {
        ...
        try {
            ...
            console.error('[DoLlynk] Fatal error:', err);
    ...
    /* ANCHOR: Mod Processing */
    const ModProcessor = {
        async applyAll(storyData) {
            // INFO: state.mods is pre-sorted by Storaage.load(), so injection order is respected
                        ...
                        for (const patch of patches) {
                            switch (patch.role) {
                                ...
                                case 'widget':
                                    ...
                                    break;
                                case 'script':
                                    JSPatcher.queue(patch);
                                    break;
                                case 'style':
                                    // INFO: Inject style immediately as it's not dependent on runtime.
                                    this._injectStyle({ ...mod, code: patch.replace, id: `${mod.id}-${patch.name}` });
                                    break;
                                default:
                                    console.warn(`[DoLlynk] Unknown patch role in ${mod.name}: ${patch.role}`);
                            }
                        }
                    } catch (e) {
    ...
    /* ANCHOR: Passage Patching */
    const PassagePatcher = {
        _normalize: str => str.replace(/\s+/g, ' ').trim(),
        apply(patch, passages) {
            ...
            for (const el of targetPassages) {
                ...
                if (findRegex) {
                    ...
                    const normalizedContent = this._normalize(content);
                    const normalizedFind = this._normalize(patch.find);
                    if (normalizedContent.includes(normalizedFind)) {
                        el.textContent = content.replace(patch.find, patch.replace);
                    } else {
    ...
    const JSPatcher = {
        ...
        applyAll() {
            ...
            console.log(`[DoLlynk] Applying ${this._queue.length} JS patches`);
            this._queue.forEach(p => this._apply(p));
            this._queue = [];
        },
        _apply(patch) {
            try {
                const funcName = patch.name;
                const target = this._findFunction(funcName);
                if (!target) {
                    console.warn(`[DoLlynk] JS Patcher: Function not found: ${funcName}`);
                    return;
                }
                const source = target.toString();
                if (patch.method === 'string' && !source.includes(patch.find)) {
                    console.warn(`[DoLlynk] JS Patcher: 'find' string not found in ${funcName}`);
                    return;
                }
                const patched = source.replace(patch.find, patch.replace);
                const container = window[funcName] ? window : window.setup;
                container[funcName] = new Function(`return ${patched}`)();
                console.log(`[DoLlynk] ✓ Patched JS function: ${funcName}`);
            } catch (err) {
                console.error('[DoLlynk] JS Patch failed:', patch, err);
            }
        },
        _findFunction: name => window[name] || window.setup?.[name]
    };
    ...
    const UI = {
        ...
        _renderMod(mod) {
            ...
            // INFO: to prevent UI crashes from corrupt data.
    ...
    const FileHandler = {
        ...
        async _installFile(file, group = null) {
            ...
            if (isBoot) {
                ...
                mod = await this.compileBootData(code, groupName);
            } else {
                const { type, name } = ModActions._getTypeAndName(file.name);
                if (!type) return;
                mod = { name, groupName: group, type, code, earlyLoad: type !== 'js', enabled: true };
            }
            if (mod) state.mods.push({ ...mod, id: crypto.randomUUID(), order: state.mods.length });
        },
        async _installZip(file) {
            try {
                ...
                if (bootFile) {
                    ...
                    const compiledMod = await this.compileBootData(bootContent, groupName, zip);
                    if (compiledMod) {
                        state.mods.push({ ...compiledMod, id: crypto.randomUUID(), order: state.mods.length });
                    }
                }

                for (const path in zip.files) {
                    const entry = zip.files[path];
                    if (entry.dir || path === bootFile?.name) continue;
        ...
        async compileBootData(bootContent, groupName, zip = null) {
            const patches = [];
            try {
                ...
                for (const addon of (bootData.addonPlugin || [])) {
                    let params = addon.params || (addon.addonName === 'ReplacePatcherAddon' ? {} : []);

                    if (addon.addonName === 'TweeReplacerAddon' && addon.paramsFiles) {
                        for (const filePath of addon.paramsFiles) {
                            const file = zip ? zip.file(new RegExp(filePath.replace(/\\/g, '/')))[0] : null;
                            if (file) {
                                try {
                                    params.push(...JSON5.parse(await file.async('text')));
                                } catch (e) { console.error(`Failed to parse external patch file ${filePath}`, e); }
                            } else { console.warn(`[DoLlynk] Patch file not found in zip: ${filePath}`); }
                        }
                    }

                    if (addon.addonName === 'TweeReplacerAddon') {
                        patches.push(...params.map(p => ({ role: 'passage', name: p.passage, method: p.isRegex ? 'regex' : 'string', find: p.findString, replace: p.replace })));
                ...
                console.error(`[DoLlynk] Failed to compile boot.json for ${groupName}`, e);
                alert(`Failed to compile boot.json for ${groupName}: ${e.message}`);
                return null;
            }
        }
    };
