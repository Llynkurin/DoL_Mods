...
(function () {
    ...
    const UI = {
        ...
        _renderMod(mod) {
            const typeLabels = { 'unified-patch': 'Patches', js: 'JS', css: 'CSS', twee: 'Twee', 'txt-content': 'Data' };
    ...
    const ModActions = {
        ...
        async saveManual() {
            ...
            const userWantsEarly = document.getElementById('mod-earlyload-check').checked;
            ...
            if (state.editingId) {
                ...
                if (isBoot) {
                    ...
                    const { type, name, earlyLoad: nameImpliesEarly } = this._getTypeAndName(fullName.split('/').pop());
                    const finalEarlyLoad = userWantsEarly || nameImpliesEarly;
                    Object.assign(modToUpdate, { name, groupName, type, code, earlyLoad: finalEarlyLoad });
                }
            } else {
                if (isBoot) {
                    ...
                    const finalEarlyLoad = userWantsEarly || nameImpliesEarly;
                    state.mods.push({ id: crypto.randomUUID(), name, groupName, type, code, earlyLoad: finalEarlyLoad, enabled: true, order: state.mods.length });
                }
            }
        ...
        _getFileNameFromMod(mod) {
            if (mod.earlyLoad && mod.type === 'js') {
                return `${mod.name}.early.js`;
            }
        ...
        _getTypeAndName(fileName) {
            let type, name = fileName, earlyLoad = false;
            if (/\.early\.js$/i.test(fileName)) {
                type = 'js';
                name = fileName.replace(/\.early\.js$/i, '');
                earlyLoad = true;
            } else if (/\.modpatch\.json5?$/i.test(fileName)) {
                type = 'unified-patch';
    ...
    const FileHandler = {
        ...
        async _installFile(file, group = null) {
            ...
            if (isBoot) {
                ...
                const { type, name, earlyLoad: nameImpliesEarly } = ModActions._getTypeAndName(file.name);
                if (!type) return;
                const earlyLoad = nameImpliesEarly || ['twee', 'css', 'unified-patch'].includes(type);
                const mod = { name, groupName: group, type, code, earlyLoad, enabled: true };
            ...
            try {
                ...
                if (bootFile) {
                    ...
                    for (const path in zip.files) {
                        ...
                        const { type, name, earlyLoad: nameImpliesEarly } = ModActions._getTypeAndName(fileName);
                        if (!type) continue;
                        const code = await entry.async('text');
                        const earlyLoad = nameImpliesEarly || ['twee', 'css', 'unified-patch'].includes(type);
                        state.mods.push({
                            id: crypto.randomUUID(), name, groupName, type, code,
                            earlyLoad, enabled: true, order: state.mods.length
                        });
                    }
                }
            } catch (err) {
            ...
            try {
                ...
                const processFileList = async (fileList, type) => {
                    ...
                    for (const filePath of fileList) {
                        ...
                        const { name, earlyLoad: nameImpliesEarly } = ModActions._getTypeAndName(file.name.split('/').pop());
                        const earlyLoad = nameImpliesEarly || ['twee', 'css', 'unified-patch'].includes(type);
                        createdMods.push({
                            name,
                            groupName: modName,
                ...
                for (const mod of unifiedPatches) {
                    try {
                        ...
                        for (const patch of patches) {
                            const fileRef = patch.replaceFile || (
                                (typeof patch.replace === 'string' && /\.(txt|twee)$/i.test(patch.replace))
                                ? patch.replace
                                : null
                            );

                            if (fileRef) {
                                const fileMod = state.mods.find(m =>
                                    m.groupName === mod.groupName && ModActions._getFileNameFromMod(m) === fileRef
                                );

                                if (fileMod) {
                                    patch.replace = fileMod.code;
                                } else {
                                    console.warn(`[DoLlynk] Replacement file not found: "${fileRef}" in group "${mod.groupName}" for patch in mod "${mod.name}".`);
                                }
                            }
