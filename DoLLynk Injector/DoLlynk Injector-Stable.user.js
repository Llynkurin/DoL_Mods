...
(async function() {
	...
	const Storage = {
		async loadMods() {
			...
			mods.forEach((mod, index) => {
				if (typeof mod.order !== 'number') mod.order = index;
			});
			mods.sort((a, b) => a.order - b.order);
			state.mods = mods;
		},
		async saveMods() {
			await GM_setValue('dollynk_mods', state.mods);
		},
		async loadOpenGroups() {
			state.openGroups = new Set(await GM_getValue('dollynk_openGroups', []));
		},
		async saveOpenGroups() {
			await GM_setValue('dollynk_openGroups', [...state.openGroups]);
		},
		async loadSettings() {
			const defaultSettings = {
				assetsEnabled: false,
				assetOverlaySystemEnabled: false
			};
		...
		async saveSettings() {
			await GM_setValue('dollynk_settings_v2', state.settings);
		},
		exportAll() {
			if (!state.mods.length) return U_WINDOW.alert('No mods to export');
			const blob = new Blob([JSON5.stringify(state.mods, null, 2)], {
		...
		importAll(file) {
			...
			reader.onload = async e => {
				try {
					...
					U_WINDOW.alert(`Import failed: ${err.message}`);
				}
			};
	...
	const Assets = {
		DB: {
			...
			async init() {
				...
				return new Promise((resolve, reject) => {
					...
					request.onupgradeneeded = (e) => {
						const store = e.target.result.createObjectStore(this.storeName, {
							keyPath: 'path'
						});
			...
			async put(path, blob, groupId) {
				await this.init();
				return this.db.transaction([this.storeName], 'readwrite').objectStore(this.storeName).put({
					path,
					groupId,
					data: blob,
					size: blob.size
		...
		Groups: {
			...
			_groups: new Map(),
			async load() {
				this._groups = new Map((await GM_getValue(this._key, [])).map(g => [g.id, g]));
			},
			async save() {
				await GM_setValue(this._key, Array.from(this._groups.values()));
			},
			get(id) {
			...
			addOrUpdate(groupData) {
				this._groups.set(groupData.id, {
					...(this.get(groupData.id) || {
						enabled: true
					}),
					...groupData
				});
		...
		Overlays: {
			...
			async save() {
				await GM_setValue(this._key, this._overlays);
			},
			getAll() {
				return this._overlays;
			},
			getEnabled() {
				return this._overlays.filter(o => o.enabled).sort((a, b) => a.priority - b.priority);
			},
		...
		FileInterceptor: {
			...
			setup() {
				...
				Object.defineProperty(HTMLImageElement.prototype, 'src', {
					...
					set(value) {
						const el = this;
						if (value && typeof value === 'string' && value.includes('img/') && !value.includes('mod_img/')) {
							self.resolveAssetPath(value, resolvedPath => {
								originalDescriptor.set.call(el, resolvedPath);
							});
						} else {
							originalDescriptor.set.call(el, value);
						}
					}
				});
				U_WINDOW.Image = function() {
				...
				if (U_DOCUMENT.body) {
					this.observer.observe(U_DOCUMENT.body, {
						childList: true,
						subtree: true,
						attributes: true,
						attributeFilter: ['src']
					...
					U_DOCUMENT.addEventListener('DOMContentLoaded', () => {
						this.observer.observe(U_DOCUMENT.body, {
							childList: true,
							subtree: true,
							attributes: true,
				...
				console.log('[✓ DoLlynk Assets] File overlay system active (Deep Hook).');
			},
		...
		UI: {
			...
			fileStrategy: {
				title: 'Local Asset Overlay Manager',
				formId: 'local-asset-manager-form',
				listId: 'local-overlay-list',
				controlId: 'dollynk-local-assets-control',
				managerButtonText: 'Manage Local Overlays',
				addButtonText: 'Add Overlay Folder',
				emptyText: 'No overlay folders configured.',
				settingsKey: 'assetOverlaySystemEnabled',
				dataManager: null,
				showManagerEvent: 'show-local-asset-manager',
				addEvent: 'add-local-overlay',
				deleteEvent: 'delete-local-overlay',
				toggleEvent: 'toggle-local-overlay',
				toggleSystemEvent: 'toggle-local-assets-system',
				events: {
					'show-local-asset-manager': () => {
						UI.showForm('local-asset-manager-form');
						Assets.UI.updateList();
					},
					'add-local-overlay': () => {
						const name = prompt("Enter a unique Asset Group Name (e.g. 'Modded image Pack').", "New Pack");
						if (!name) return;
						const source = prompt("Enter the Relative Path (folder next to HTML, e.g. 'mod_img/hair/' to target the hair folder).", "mod_img/");
						if (!source) return;
						const target = prompt("Enter the Replacement Target (game folder, e.g. 'img/hair/' to target the hair folder).", "img/");
						if (!target) return;
						Assets.Overlays.add(name.trim(), source.trim(), target.trim());
						Assets.UI.updateList();
						state.needsReload = true;
						UI.updateModList();
					},
					'edit-local-overlay': (_, id) => {
						const overlay = Assets.Overlays.getAll().find(o => o.id === id);
						if (!overlay) return;
						const name = prompt("Asset Group Name:", overlay.name);
						if (!name) return;
						const source = prompt("Relative Path (folder next to HTML):", overlay.source);
						if (!source) return;
						const target = prompt("Replacement Target (game folder):", overlay.target);
						if (!target) return;
						Assets.Overlays.update(id, {
							name: name.trim(),
							source: source.trim(),
							target: target.trim()
						});
					...
					'delete-local-overlay': (_, id) => {
						...
						if (o && confirm(`Remove "${o.name}" overlay?`)) {
							Assets.Overlays.remove(id);
							Assets.UI.updateList();
							state.needsReload = true;
							UI.updateModList();
						}
					},
					'toggle-local-overlay': (_, id) => {
						Assets.Overlays.toggle(id);
						Assets.UI.updateList();
						state.needsReload = true;
						UI.updateModList();
					},
					'toggle-local-assets-system': async () => {
						state.settings.assetOverlaySystemEnabled = !state.settings.assetOverlaySystemEnabled;
						await Storage.saveSettings();
						state.needsReload = true;
						UI.updateModList();
						Assets.UI.updateControl();
					},
				}
			},
			webStrategy: {
				title: 'Asset Manager',
				formId: 'mod-asset-manager-form',
				listId: 'asset-group-list',
				controlId: 'dollynk-assets-control',
				managerButtonText: 'Manage Assets',
				addButtonText: 'Import Folder',
				emptyText: 'No asset groups installed.',
				settingsKey: 'assetsEnabled',
				dataManager: null,
				showManagerEvent: 'show-asset-manager',
				addEvent: 'import-assets',
				deleteEvent: 'delete-asset-group',
				toggleEvent: 'toggle-asset-group',
				toggleSystemEvent: 'toggle-assets-system',
				events: {
					'show-asset-manager': () => {
						UI.showForm('mod-asset-manager-form');
						Assets.UI.updateList();
					},
					'import-assets': () => {
						const input = Object.assign(U_DOCUMENT.createElement('input'), {
							type: 'file',
						...
						input.onchange = async e => {
							...
							for (const file of files) {
								await Assets.DB.put(Assets._normalizePath(file.webkitRelativePath), file, groupId);
							}
							...
							UI.hideLoader();
							U_WINDOW.alert(`Imported ${groupData.length} assets into "${groupId}".`);
							Assets.UI.updateList();
						};
						input.click();
					},
					'delete-asset-group': async (_, id) => {
						...
						if (!g || !confirm(`Delete asset group "${g.id}"? This permanently removes ${g.fileCount} files.`)) return;
						UI.showLoader('Deleting...');
						await Assets.DB.deleteByGroupId(g.id);
						Assets.Groups.remove(g.id);
						UI.hideLoader();
						Assets.UI.updateList();
					},
					'toggle-asset-group': (_, id) => {
						Assets.Groups.toggle(id);
						Assets.UI.updateList();
					},
					'expand-asset-group': (_, id) => {
						Assets.UI.expandedGroups.has(id) ? Assets.UI.expandedGroups.delete(id) : Assets.UI.expandedGroups.add(id);
						Assets.UI.updateList();
					},
					'toggle-assets-system': async () => {
						state.settings.assetsEnabled = !state.settings.assetsEnabled;
						await Storage.saveSettings();
						state.needsReload = true;
						UI.updateModList();
						Assets.UI.updateControl();
	...
	const FileHandler = {
		browse() {
			...
			i.type = 'file';
			i.multiple = true;
			i.accept = '.js,.css,.twee,.json,.json5,.txt';
		...
		async installStaged() {
			...
			for (const file of state.stagedFiles) {
				if (/\.(json5?)$/i.test(file.name)) await this._installJson(file);
		...
		async _installJson(file) {
			try {
				...
				if (newMods.length > 0) newMods.forEach(mod => state.mods.push({
					...mod,
					order: state.mods.length
				}));
			} catch (err) {
				U_WINDOW.alert(`Failed to import ${file.name}: ${err.message}`);
			}
		},
		async _installZip(file) {
			try {
				...
				U_WINDOW.alert(`Failed to install ${file.name}: ${err.message}`);
			}
		}
	};
	...
	const ModActions = {
			...
			if (U_WINDOW.confirm('Delete this mod?')) {
				this._updateAndSave(() => {
					if (id === 'undefined') {
						...
						if (indexToDelete > -1) {
							state.mods.splice(indexToDelete, 1);
						}
					} else {
		...
		async deleteGroup(groupId) {
			...
			if (U_WINDOW.confirm(`Delete all mods in group "${groupId}"${assetInfo}? This is permanent.`)) {
				if (assetGroup) {
					await Assets.DB.deleteByGroupId(groupId);
					Assets.Groups.remove(groupId);
				}
				this._updateAndSave(() => {
					state.mods = state.mods.filter(m => !(m.groupName === groupId || m.groupName?.startsWith(groupId + '/')));
				});
			}
		},
		async renameGroup(oldName) {
			...
			this._updateAndSave(() => {
				...
				[...state.openGroups].filter(g => g === oldName || g.startsWith(oldName + '/')).forEach(g => {
					state.openGroups.delete(g);
					state.openGroups.add(newName + g.substring(oldName.length));
				});
		...
		addToGroup(id) {
			UI.showForm('mod-manual-form');
			UI.dom.nameInput.value = `${id}/`;
			UI.dom.formHeader.textContent = `Add to ${id}`;
		},
		saveManual() {
			const fullName = UI.dom.nameInput.value.trim();
			const code = UI.dom.codeInput.value;
			if (!fullName || !code) return;
			const {
				type,
				name,
				earlyLoad
			} = this._getTypeAndName(fullName.split('/').pop());
			...
			const finalEarlyLoad = UI.dom.earlyLoadCheck.checked || earlyLoad;
			this._updateAndSave(() => {
				if (state.editingId) {
					const mod = state.mods.find(m => m.id === state.editingId);
					if (mod) Object.assign(mod, {
						name,
						groupName,
						type,
						code,
						earlyLoad: finalEarlyLoad
					});
				} else {
					state.mods.push({
						id: crypto.randomUUID(),
		...
		exportMod(id) {
			const mod = state.mods.find(m => m.id === id);
			if (mod) _triggerDownload(new Blob([mod.code], {
				type: 'text/plain;charset=utf-8'
			}), this._getFileNameFromMod(mod));
		},
		async exportGroup(id, button) {
			...
			try {
				_triggerDownload(new Blob([JSON5.stringify(mods, null, 2)], {
					type: 'application/json5'
				}), `${id.split('/').pop()}.dollynk-group.json5`);
			} catch (err) {
				console.error(`[❌ DoLlynk] Failed to generate JSON5 for group ${id}:`, err);
			} finally {
	...
	const UI = {
		...
		forceInjectManager() {
			...
			if (!overlay) {
				overlay = U_DOCUMENT.createElement('div');
				overlay.id = 'customOverlay';
				overlay.className = 'overlay';
				U_DOCUMENT.body.appendChild(overlay);
			}
			let content = U_DOCUMENT.getElementById('customOverlayContent');
			if (!content) {
				content = U_DOCUMENT.createElement('div');
		...
		updateModList() {
			...
			topLevelGroups.forEach(groupName => this._renderItem({
				isGroup: true,
				name: groupName
			}, 0, buffer));
		...
		_renderItem(item, depth, buffer) {
			...
			if (item.isGroup) {
				...
				directSubgroups.forEach(sg => this._renderItem({
					isGroup: true,
					name: sg
				}, depth + 1, buffer));
				...
				const types = {
					'unified-patch': 'Patches',
					js: 'JS',
					css: 'CSS',
					twee: 'Twee',
					'txt-content': 'Data'
				};
				const typeLabel = types[item.type] || item.type?.toUpperCase() || 'INVALID';
		...
		showLoader(text) {
			...
			if (!loader) {
				loader = U_DOCUMENT.createElement('div');
		...
		showForm(formId) {
			...
			const formToShow = U_DOCUMENT.getElementById(formId);
		...
		resetForm() {
			...
			this.dom.earlyLoadCheck.checked = false;
			this.dom.formHeader.textContent = 'Add Mod Manually';
		},
		_getManagerTemplate() {
	...
	const Events = {
		bind(container) {
			...
			UI.dom.dropzone.addEventListener('drop', e => {
				...
				FileHandler.stage(Array.from(e.dataTransfer.files));
			});
		...
		bindDragDrop(container) {
			...
			container.addEventListener('dragleave', e => {
				if (!e.currentTarget.contains(e.relatedTarget)) cleanup();
			});
		...
		_handlers: {
			...
			'import-all': () => {
				...
				i.type = 'file';
				i.accept = '.json,.json5';
				i.onchange = e => e.target.files[0] && Storage.importAll(e.target.files[0]);
				i.click();
			},
	...
	const TweeProcessor = {
		inject(content, storyData, source) {
			...
			let count = 0;
			const passages = trimmedContent.split(/\n::/m);
			for (let i = 1; i < passages.length; i++) {
				const text = passages[i];
				if (!text.trim()) continue;
				const passageData = this._parsePassage(text, source);
				if (passageData) {
					this._injectPassage(passageData, storyData, fragment);
					count++;
				}
			}
		...
		_parsePassage(text, source) {
			...
			if (!match?.[1]) {
				console.warn(`[⚠️ DoLlynk] Invalid Twee Header in ${source}: ${header}`);
				return null;
			}
		...
		_injectPassage({
			...
			if (existing) {
				...
				if (tags) existing.setAttribute('tags', tags);
				else existing.removeAttribute('tags');
	...
	const PassagePatcher = {
		apply(patch, passageMap) {
			...
			const targets = [];
			if (patch.method === 'regex') {
				const nameRegex = new RegExp(patch.name);
			...
			for (const el of targets) {
				...
				const isRegex = patch.isRegex || patch.method === 'regex';
				const findExpr = isRegex ? new RegExp(patch.find, 'g') : patch.find;
				const hasMatch = isRegex ? findExpr.test(content) : content.includes(findExpr);
				if (hasMatch) {
					el.textContent = content.replaceAll(findExpr, patch.replace);
					results.success++;
				} else {
					results.failed++;
				}
	...
	const ModProcessor = {
		async applyAll(storyData) {
			if (U_WINDOW.DoLlynk.preprocessors?.length > 0) {
				...
				for (const processor of U_WINDOW.DoLlynk.preprocessors) {
					try {
						processor(state);
					} catch (e) {
						console.error('[❌ DoLlynk] Pre-processor failed:', e);
					}
				}
			}
			...
			for (const mod of allMods) {
				if (mod.type === 'twee') {
					injectedPassageCount += TweeProcessor.inject(mod.code, fragment, `${mod.groupName || ''}/${mod.name}`);
				}
			}
			if (injectedPassageCount > 0) {
				storyData.appendChild(fragment);
				for (const p of fragment.querySelectorAll('tw-passagedata')) {
					passageMap.set(p.getAttribute('name'), p);
				}
				console.log(`[✓ DoLlynk Twee] Injected ${injectedPassageCount} passages.`);
			}
			UI.showLoader('Phase 2: Patching');
			for (const mod of allMods) {
				if (mod.type === 'unified-patch') this._applyUnifiedPatches(mod, passageMap);
			}
		...
		_applyUnifiedPatches(mod, passageMap) {
			...
			try {
				...
				if (stats.fail > 0) {
					console.warn(`[⚠️ DoLlynk Patch] ${mod.name}: Failed to apply ${stats.fail} patch(es).`);
					stats.failed.forEach(n => console.warn(`  └─ Patch: "${n}"`));
				}
			} catch (e) {
				console.error(`[❌ DoLlynk] Error parsing patch file "${mod.name}":`, e);
			}
		},
		injectRuntime() {
			const runtimeMods = state.mods.filter(m => m.enabled && !m.earlyLoad && m.type === 'js');
			for (const mod of runtimeMods) {
				this._injectScript(mod, 'runtime');
			}
	...
	observer.observe(U_DOCUMENT.documentElement, {
		childList: true,
		subtree: true
	});
	...
	U_DOCUMENT.addEventListener('keydown', (e) => {
		if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'm') {
			...
			try {
				...
				console.error('[❌ DoLlynk] Failsafe UI Error:', err);
			}
		}
	}, true);
