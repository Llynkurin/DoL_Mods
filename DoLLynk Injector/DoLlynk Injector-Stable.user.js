...
(async function() {
	...
	// OPTIMIZATION: Cache navigator properties to avoid repeated lookups
	const UA = U_WINDOW.navigator.userAgent;
	const ENV = {
		isFirefox: UA.includes("Firefox"),
		isChromium: !!U_WINDOW.chrome && !UA.includes("Firefox"),
	...
	/* ANCHOR: Advanced APIs & Hooks */
	const Hooks = {
		_subscribers: {
			':storyready': [], ':passageinit': [], ':passagestart': [],
			':passagerender': [], ':passagedisplay': [], ':passageend': [],
			'wikify:before': [], 'wikify:after': []
		},
		subscribe(hookName, callback) {
			if (this._subscribers[hookName]) this._subscribers[hookName].push(callback);
		},
		_trigger(hookName, ...args) {
			const subs = this._subscribers[hookName];
			if (subs && subs.length) {
				for (let i = 0; i < subs.length; i++) {
					try { subs[i](...args); }
					catch (e) { console.error(`[❌ DoLlynk Hook Error] '${hookName}':`, e); }
				}
			}
		},
		_patchSugarcube() {
			const $ = U_WINDOW.jQuery || U_WINDOW.$;
			if (!$) return;
			const eventNames = [':passageinit', ':passagestart', ':passagerender', ':passagedisplay', ':passageend'];
			eventNames.forEach(name => $(U_DOCUMENT).on(name, (ev) => this._trigger(name, ev)));
			$(U_DOCUMENT).one(':storyready', (ev) => this._trigger(':storyready', ev));

			if (U_WINDOW.Wikifier?.prototype?.wikify) {
				const originalWikify = U_WINDOW.Wikifier.prototype.wikify;
				U_WINDOW.Wikifier.prototype.wikify = function(source, ...args) {
					Hooks._trigger('wikify:before', { source, output: this.output, wikifier: this });
					originalWikify.call(this, source, ...args);
					Hooks._trigger('wikify:after', { source, output: this.output, wikifier: this });
				};
				console.log('[✓ DoLlynk API] Wikify hooks installed.');
			}
		}
	};
	...
	const Assets = {
		DB: {
			...
			async init() {
				...
				this._initPromise = new Promise((resolve, reject) => {
					...
					request.onerror = () => { this._initPromise = null; reject(request.error); };
					request.onsuccess = () => { this.db = request.result; resolve(); };
					request.onupgradeneeded = (e) => {
						const db = e.target.result;
						const store = !db.objectStoreNames.contains(this.storeName) ?
							db.createObjectStore(this.storeName, { keyPath: 'path' }) :
							e.target.transaction.objectStore(this.storeName);
			...
			async get(path) {
				await this.init();
				return new Promise(r => this.db.transaction([this.storeName]).objectStore(this.storeName).get(path).onsuccess = e => r(e.target.result));
			},
			async getByGroupId(groupId) {
				await this.init();
				return new Promise(r => this.db.transaction([this.storeName]).objectStore(this.storeName).index('groupId').getAll(groupId).onsuccess = e => r(e.target.result));
			},
			async put(path, blob, groupId) {
				await this.init();
				return this.db.transaction([this.storeName], 'readwrite').objectStore(this.storeName).put({ path, groupId, data: blob, size: blob.size });
			},
		...
		Groups: {
			...
			addOrUpdate(groupData) {
				this._groups.set(groupData.id, { ...(this.get(groupData.id) || { enabled: true }), ...groupData });
				this.save();
			},
			remove(id) { this._groups.delete(id); this.save(); },
			toggle(id) {
				const g = this.get(id);
				if (g) { g.enabled = !g.enabled; this.save(); }
			},
			isEnabled(id) { return this.get(id)?.enabled ?? false; },
		},
		Overlays: {
			_key: 'dollynk_asset_overlays_v1',
			_overlays: [],
			async load() {
				if (typeof state.settings.assetOverlayEnabled !== 'undefined') {
					if (state.settings.assetOverlayEnabled) this._overlays.push({
						id: 'default_mod_img', name: 'Default (mod_img)', source: 'mod_img/', target: 'img/', enabled: true, priority: 0
					});
					delete state.settings.assetOverlayEnabled;
					state.settings.assetOverlaySystemEnabled = true;
					await Storage.saveSettings();
				} else {
					this._overlays = await GM_getValue(this._key, []);
				}
				this._overlays.forEach((o, i) => o.priority = i);
				await this.save();
			},
			...
			add(name, source, target) {
				const newOverlay = {
					id: source.replace(/[^a-zA-Z0-9]/g, '_') + Date.now(),
					name,
					source: source.endsWith('/') ? source : source + '/',
					target: target.endsWith('/') ? target : target + '/',
					enabled: true,
					priority: this._overlays.length
				};
			...
			toggle(id) {
				const o = this._overlays.find(ov => ov.id === id);
				if (o) { o.enabled = !o.enabled; this.save(); }
			},
		},
		...
		WebInterceptor: {
			setup() {
				...
				const handler = (el, path) => {
					...
					// OPTIMIZATION: Use Base64 via getDataURL for CSP compatibility
					Assets.getDataURL(path).then(uri => desc.set.call(el, uri || path));
				};
				Object.defineProperty(HTMLImageElement.prototype, 'src', {
					set(v) { handler(this, v); },
					get: desc.get
				});
				...
				console.log(`[✓ DoLlynk Assets] Web protocol image interceptor active (Base64 CSP Safe).`);
			}
		},
		FileInterceptor: {
			pathCache: new Map(),
			pendingImages: new Map(),
			negativeCache: new Set(), // OPTIMIZATION: Cache missing files to avoid IO
			OriginalImage: null,
			observer: null,
			setup() {
				...
				// OPTIMIZATION: Helper to check if processing is needed
				const needsProcessing = (v) => v && typeof v === 'string' && v.includes('img/') && !v.includes('mod_img/');

				Object.defineProperty(HTMLImageElement.prototype, 'src', {
					get() { return originalDescriptor.get.call(this); },
					set(value) {
						if (needsProcessing(value)) {
							self.resolveAssetPath(value, resolvedPath => originalDescriptor.set.call(this, resolvedPath));
						} else {
				...
				U_WINDOW.Image = function() {
					return new self.OriginalImage();
				};
				U_WINDOW.Image.prototype = this.OriginalImage.prototype;

				// OPTIMIZATION: Reduced observer scope
				this.observer = new MutationObserver((mutations) => {
					for (const mutation of mutations) {
						if (mutation.type === 'childList') {
							for (const node of mutation.addedNodes) {
								if (node.nodeName === 'IMG') self._checkNode(node);
								else if (node.nodeType === 1 && node.getElementsByTagName) {
									// OPTIMIZATION: Only query if node might contain images
									if (node.querySelector) {
										const imgs = node.querySelectorAll('img');
										for (let i = 0; i < imgs.length; i++) self._checkNode(imgs[i]);
									}
								}
							}
						} else if (mutation.type === 'attributes') {
							self._checkNode(mutation.target);
						}
					}
				});

				const obsConfig = { childList: true, subtree: true, attributes: true, attributeFilter: ['src'] };
				if (U_DOCUMENT.body) this.observer.observe(U_DOCUMENT.body, obsConfig);
				else U_DOCUMENT.addEventListener('DOMContentLoaded', () => this.observer.observe(U_DOCUMENT.body, obsConfig));

				console.log('[✓ DoLlynk Assets] File overlay system active (Deep Hook Optimized).');
			},
			_checkNode(img) {
				const rawSrc = img.getAttribute('src');
				if (rawSrc && rawSrc.includes('img/') && !rawSrc.includes('mod_img/')) {
					this.resolveAssetPath(rawSrc, (resolved) => {
			...
			resolveAssetPath(originalPath, callback) {
				...
				if (this.negativeCache.has(normalizedPath)) return callback(originalPath); // Skip IO
				...
				let i = 0;
				const tryNextOverlay = () => {
					if (i >= activeOverlays.length) {
						this.negativeCache.add(normalizedPath); // Remember failure
						return this.finalizePath(normalizedPath, normalizedPath);
					}
					const overlay = activeOverlays[i++];
					const target = overlay.target.replace(/\\/g, '/');
					if (normalizedPath.includes(target)) {
						const relative = normalizedPath.substring(normalizedPath.indexOf(target) + target.length);
						const modPath = (overlay.source + relative).replace(/\\/g, '/');

						// OPTIMIZATION: Only new Image() if strictly needed
						const testImg = new this.OriginalImage();
						const timer = setTimeout(() => {
							testImg.onload = null; testImg.onerror = null; tryNextOverlay();
						}, 200);
						testImg.onload = () => {
							clearTimeout(timer);
			...
			finalizePath(originalKey, finalPath) {
				this.pathCache.set(originalKey, finalPath);
				const callbacks = this.pendingImages.get(originalKey) || [];
				this.pendingImages.delete(originalKey);
				callbacks.forEach(cb => cb(finalPath));
			}
		},
		cache: new Map(), // Base64 Cache
		_normalizePath(relativePath) {
			let p = relativePath.replace(/^[/\\]+/g, '');
			const i = p.indexOf('img/');
			return i > -1 ? p.substring(i) : `img/${p}`;
		},
		async getDataURL(path) { // Web - Reverted to Base64 for CSP
			...
			if (asset && this.Groups.isEnabled(asset.groupId)) {
				return new Promise(r => {
					const reader = new FileReader();
					reader.onload = () => {
						// OPTIMIZATION: Cache the Base64 string to avoid re-encoding overhead
						this.cache.set(normalized, reader.result);
						r(reader.result);
					};
		...
		UI: {
			...
			updateControl() {
				U_DOCUMENT.getElementById(this.strategy.controlId)?.classList.toggle('enabled', state.settings[this.strategy.settingsKey]);
			},
			async updateList() {
				const container = U_DOCUMENT.getElementById(this.strategy.listId);
				const items = await this.strategy.dataManager.getAll();
				if (!items.length) {
					container.innerHTML = `<div class="settingsToggleItem">${this.strategy.emptyText}</div>`;
					return;
				}
			...
			async renderFolderTree(groupId) {
				...
				const tree = {};
				assets.forEach(a => a.path.split('/').reduce((o, p, i, arr) => o[p] = o[p] || (i === arr.length - 1 ? null : {}), tree));
			...
			fileStrategy: {
				title: 'Local Asset Overlay Manager', formId: 'local-asset-manager-form', listId: 'local-overlay-list', controlId: 'dollynk-local-assets-control',
				managerButtonText: 'Manage Local Overlays', addButtonText: 'Add Overlay Folder', emptyText: 'No overlay folders configured.',
				settingsKey: 'assetOverlaySystemEnabled', dataManager: null, showManagerEvent: 'show-local-asset-manager', addEvent: 'add-local-overlay',
				deleteEvent: 'delete-local-overlay', toggleEvent: 'toggle-local-overlay', toggleSystemEvent: 'toggle-local-assets-system',
				events: {
					...
					'add-local-overlay': () => {
						...
						const source = prompt("Enter the Relative Path (folder next to HTML, e.g. 'mod_img/hair/').", "mod_img/"); if (!source) return;
						const target = prompt("Enter the Replacement Target (game folder, e.g. 'img/hair/').", "img/"); if (!target) return;
					...
					'delete-local-overlay': (_, id) => {
						const o = Assets.Overlays.getAll().find(ov => ov.id === id);
						if (o && confirm(`Remove "${o.name}" overlay?`)) { Assets.Overlays.remove(id); Assets.UI.updateList(); state.needsReload = true; UI.updateModList(); }
					},
					'toggle-local-overlay': (_, id) => { Assets.Overlays.toggle(id); Assets.UI.updateList(); state.needsReload = true; UI.updateModList(); },
					'toggle-local-assets-system': async () => {
						state.settings.assetOverlaySystemEnabled = !state.settings.assetOverlaySystemEnabled; await Storage.saveSettings(); state.needsReload = true; UI.updateModList(); Assets.UI.updateControl();
					},
				}
			},
			webStrategy: {
				title: 'Asset Manager', formId: 'mod-asset-manager-form', listId: 'asset-group-list', controlId: 'dollynk-assets-control',
				managerButtonText: 'Manage Assets', addButtonText: 'Import Folder', emptyText: 'No asset groups installed.',
				settingsKey: 'assetsEnabled', dataManager: null, showManagerEvent: 'show-asset-manager', addEvent: 'import-assets',
				deleteEvent: 'delete-asset-group', toggleEvent: 'toggle-asset-group', toggleSystemEvent: 'toggle-assets-system',
				events: {
					...
					'import-assets': () => {
						...
						input.onchange = async e => {
							...
							const groupId = prompt(`Enter name for this asset group:`, files[0]?.webkitRelativePath.split('/')[0] || 'CustomAssets');
							if (!groupId) return;
					...
					'delete-asset-group': async (_, id) => {
						const g = Assets.Groups.get(id);
						if (!g || !confirm(`Delete asset group "${g.id}"?`)) return;
						UI.showLoader('Deleting...'); await Assets.DB.deleteByGroupId(g.id); Assets.Groups.remove(g.id); UI.hideLoader(); Assets.UI.updateList();
					},
					...
					'toggle-assets-system': async () => {
						state.settings.assetsEnabled = !state.settings.assetsEnabled; await Storage.saveSettings(); state.needsReload = true; UI.updateModList(); Assets.UI.updateControl();
					},
				}
			}
		},
	...
	const FileHandler = {
		browse() {
			const i = U_DOCUMENT.createElement('input');
			i.type = 'file'; i.multiple = true; i.accept = '.js,.css,.twee,.json,.json5,.txt';
			i.onchange = e => this.stage(Array.from(e.target.files));
			i.click();
		},
		stage(files) {
			const valid = files.filter(f => /\.(js|css|twee|json5?|zip|txt)$/i.test(f.name));
			state.stagedFiles.push(...valid.filter(f => !state.stagedFiles.some(s => s.name === f.name)));
			this._updateStageView();
		},
		unstage(index) { state.stagedFiles.splice(index, 1); this._updateStageView(); },
		_updateStageView() {
			if (UI.dom.stagedFiles) UI.dom.stagedFiles.innerHTML = state.stagedFiles.map((f, i) => `<div class="settingsToggleItem"><span>${f.name}</span><button data-action="unstage-file" data-index="${i}">Remove</button></div>`).join('');
		},
		async installStaged() {
			if (!state.stagedFiles.length) return;
			UI.showLoader('Installing...');
			for (const file of state.stagedFiles) {
				const isExplicitPatch = /\.modpatch\.json5?(\.txt)?$/i.test(file.name);
				if (isExplicitPatch) await this._installFile(file, null);
				else if (/\.json5?(\.txt)?$/i.test(file.name)) await this._installJson(file);
				else await this._installFile(file, null);
			}
			state.stagedFiles = []; state.needsReload = true;
			await Storage.saveMods(); UI.resetForm(); UI.updateModList(); UI.hideLoader();
		},
		async _installFile(file, groupName) {
			const { type, name, earlyLoad } = ModActions._getTypeAndName(file.name);
			if (!type) return;
			const code = await file.text();
			state.mods.push({
				name, groupName, type, code,
				earlyLoad: earlyLoad || ['twee', 'css', 'unified-patch'].includes(type),
				enabled: true, id: crypto.randomUUID(), order: state.mods.length
			});
		},
		async _installJson(file) {
			try {
				const content = await file.text();
				const imported = JSON5.parse(content);
				if (!Array.isArray(imported)) return;
				const isModBundle = imported.every(i => i.type && (i.code !== undefined || i.type === 'txt-content'));
				if (isModBundle) {
					...
					const { name } = ModActions._getTypeAndName(file.name);
					state.mods.push({
						id: crypto.randomUUID(), name, type: 'unified-patch',
						code: content, earlyLoad: true, enabled: true, order: state.mods.length
					});
				}
		...
		async _installZip(file) {
			try {
				...
				if (!ENV.isFileProtocol && state.settings.assetsEnabled) {
					const count = await Assets.importFromZip(zip, rootGroup);
					if (count > 0) console.log(`[✓ DoLlynk Assets] Imported ${count} assets from ${file.name}.`);
				}
	...
	const ModActions = {
		...
		toggleGroup(id) {
			this._updateAndSave(() => {
				const mods = state.mods.filter(m => m.groupName === id || m.groupName?.startsWith(id + '/'));
				const newState = !mods.every(m => m.enabled);
				mods.forEach(m => m.enabled = newState);
			});
		},
		setAllEnabled(enabled) {
			if (U_WINDOW.confirm(`This will ${enabled ? 'enable' : 'disable'} all ${state.mods.length} mods. Continue?`)) {
				this._updateAndSave(() => state.mods.forEach(m => m.enabled = enabled));
			}
		},
		delete(id) {
			if (U_WINDOW.confirm('Delete this mod?')) {
				this._updateAndSave(() => {
					if (id === 'undefined') {
						const indexToDelete = state.mods.findIndex(m => m.id == null);
						if (indexToDelete > -1) state.mods.splice(indexToDelete, 1);
					} else { state.mods = state.mods.filter(m => m.id !== id); }
					if (state.editingId === id) UI.resetForm();
				});
			}
		},
		async deleteGroup(groupId) {
			let assetInfo = ''; let assetGroup = null;
			if (!ENV.isFileProtocol) {
				assetGroup = Assets.Groups.get(groupId);
				if (assetGroup) assetInfo = ` and its ${assetGroup.fileCount} linked assets`;
			}
		...
		async renameGroup(oldName) {
			...
			this._updateAndSave(() => {
				state.mods.forEach(mod => {
					if (mod.groupName === oldName) mod.groupName = newName;
					else if (mod.groupName?.startsWith(oldName + '/')) mod.groupName = newName + mod.groupName.substring(oldName.length);
				});
				[...state.openGroups].filter(g => g === oldName || g.startsWith(oldName + '/')).forEach(g => {
					state.openGroups.delete(g); state.openGroups.add(newName + g.substring(oldName.length));
				});
				Storage.saveOpenGroups();
			});
		},
		edit(id) {
			const mod = state.mods.find(m => m.id === id); if (!mod) return;
			UI.showForm('mod-manual-form'); state.editingId = id;
			UI.dom.nameInput.value = (mod.groupName ? `${mod.groupName}/` : '') + this._getFileNameFromMod(mod);
			UI.dom.codeInput.value = mod.code;
			UI.dom.earlyLoadCheck.checked = !!mod.earlyLoad;
			UI.dom.formHeader.textContent = 'Edit Mod';
		},
		addToGroup(id) { UI.showForm('mod-manual-form'); UI.dom.nameInput.value = `${id}/`; UI.dom.formHeader.textContent = `Add to ${id}`; },
		saveManual() {
			const fullName = UI.dom.nameInput.value.trim(); const code = UI.dom.codeInput.value;
			if (!fullName || !code) return;
			const { type, name, earlyLoad } = this._getTypeAndName(fullName.split('/').pop());
			if (!type) return U_WINDOW.alert('Invalid file extension.');
			const groupName = fullName.includes('/') ? fullName.substring(0, fullName.lastIndexOf('/')) : null;
			this._updateAndSave(() => {
				const modData = { name, groupName, type, code, earlyLoad: UI.dom.earlyLoadCheck.checked || earlyLoad };
				if (state.editingId) {
					const mod = state.mods.find(m => m.id === state.editingId);
					if (mod) Object.assign(mod, modData);
				} else {
					state.mods.push({ id: crypto.randomUUID(), ...modData, enabled: true, order: state.mods.length });
				}
			});
			UI.resetForm();
		},
		reorder(dragged, target) {
			this._updateAndSave(() => {
				const [dragType, dragId] = dragged.split(':');
				...
				let itemsToMove;
				if (dragType === 'mod') {
					const i = state.mods.findIndex(m => m.id === dragId);
					if (i > -1) itemsToMove = state.mods.splice(i, 1);
				} else {
					itemsToMove = state.mods.filter(m => m.groupName === dragId || m.groupName?.startsWith(dragId + '/'));
					state.mods = state.mods.filter(m => !itemsToMove.includes(m));
				}
				if (!itemsToMove?.length) return;
				let finalIndex = getTargetIndex();
				state.mods.splice(finalIndex > -1 ? finalIndex : state.mods.length, 0, ...itemsToMove);
		...
		exportMod(id) {
			...
			if (mod) {
				...
				if (mod.type === 'unified-patch') { try { content = JSON5.stringify(JSON5.parse(content), null, 2); } catch (e) {} }
				_triggerDownload(new Blob([content], { type: 'text/plain;charset=utf-8' }), this._getFileNameFromMod(mod));
			}
		},
		async exportGroup(id, button) {
			...
			try {
				_triggerDownload(new Blob([JSON5.stringify(mods, null, 2)], { type: 'application/json5' }), `${id.split('/').pop()}.mod.json5`);
		...
		_getFileNameFromMod(mod) {
			const extMap = { 'unified-patch': '.modpatch.json5', 'twee': '.twee', 'js': '.js', 'css': '.css', 'txt-content': '.txt' };
			return `${mod.name}${mod.earlyLoad && mod.type === 'js' ? '.early.js' : extMap[mod.type] || `.${mod.type}`}`;
		},
		_getTypeAndName(fileName) {
			const rules = [
				{ r: /\.early\.js$/i, t: 'js', e: true }, { r: /\.modpatch\.json5?(\.txt)?$/i, t: 'unified-patch' },
				{ r: /\.twee(\.txt)?$/i, t: 'twee' }, { r: /\.js$/i, t: 'js' }, { r: /\.css$/i, t: 'css' },
				{ r: /\.txt$/i, t: 'txt-content' }, { r: /\.json5?$/i, t: 'unified-patch' }
			];
			for (const { r, t, e } of rules) {
				if (r.test(fileName)) return { type: t, name: fileName.replace(r, ''), earlyLoad: !!e };
			}
			return { type: null, name: fileName, earlyLoad: false };
		}
	};
	...
	const UI = {
		...
		initializeInjectedHTML(container) {
			...
			container.id = 'mod-manager-wrapper';
			container.innerHTML = this._getManagerTemplate();
			this.dom = {
				list: container.querySelector('#mod-list-section'),
				reloadNotice: container.querySelector('#mod-reload-actions'),
				stagedFiles: container.querySelector('#mod-staged-files'),
				mainScreen: container.querySelector('#dollynk-main-screen'),
				manualForm: container.querySelector('#mod-manual-form'),
				formHeader: container.querySelector('#form-header'),
				nameInput: container.querySelector('#mod-name-input'),
				codeInput: container.querySelector('#mod-code-input'),
				earlyLoadCheck: container.querySelector('#mod-earlyload-check'),
				dropzone: container.querySelector('.mod-dropzone')
			};
			Events.bind(container);
			this.updateModList();
		},
		forceInjectManager() {
			let overlay = U_DOCUMENT.getElementById('customOverlay');
			if (!overlay) {
				overlay = U_DOCUMENT.createElement('div'); overlay.id = 'customOverlay'; overlay.className = 'overlay'; U_DOCUMENT.body.appendChild(overlay);
			}
			let content = U_DOCUMENT.getElementById('customOverlayContent');
			if (!content) {
				content = U_DOCUMENT.createElement('div'); content.id = 'customOverlayContent'; overlay.appendChild(content);
			}
			if (!U_DOCUMENT.getElementById('mod-manager-wrapper')) {
				content.innerHTML = '<div class="settingsHeader options">Mod Manager (Failsafe)</div><div id="mod-manager-wrapper-placeholder"></div>';
				this.initializeInjectedHTML(U_DOCUMENT.getElementById('mod-manager-wrapper-placeholder'));
			}
			overlay.style.display = 'block';
		},
		updateModList() {
			if (!this.dom.list) return;
			this.dom.reloadNotice.style.display = state.needsReload ? 'block' : 'none';
			const standalone = state.mods.filter(m => !m.groupName);
			const topLevelGroups = [...new Set(state.mods.filter(m => m.groupName).map(m => m.groupName.split('/')[0]))];
			const buffer = [];
			standalone.forEach(mod => this._renderItem(mod, 0, buffer));
			topLevelGroups.forEach(groupName => this._renderItem({ isGroup: true, name: groupName }, 0, buffer));
		...
		_getGroupState(groupName) {
			...
			if (mods.every(m => m.enabled)) return 'enabled';
			if (mods.every(m => !m.enabled)) return 'disabled';
			return 'mixed';
		},
		_renderItem(item, depth, buffer) {
			...
			if (item.isGroup) {
				...
				const groupState = this._getGroupState(groupName);
				if (groupState === 'empty') return;
				const isOpen = state.openGroups.has(groupName);
				const displayName = groupName.split('/').pop();
				...
				directSubgroups.forEach(sg => this._renderItem({ isGroup: true, name: sg }, depth + 1, buffer));
				directMods.forEach(m => this._renderItem(m, depth + 1, buffer));
				buffer.push(`</div></details>`);
			} else {
				const typeLabel = { 'unified-patch': 'Patches', js: 'JS', css: 'CSS', twee: 'Twee', 'txt-content': 'Data' }[item.type] || item.type?.toUpperCase() || 'INVALID';
				buffer.push(`<div class="settingsToggleItem dollynk-mod-item ${item.earlyLoad ? 'mod-item-early' : ''}" data-mod-id="${item.id}" data-state="${item.enabled ? 'enabled' : 'disabled'}"><div class="mod-info" ${indent}>${item.name}<span class="mod-type">${typeLabel}</span>${typeLabel === 'INVALID' ? '<span class="mod-badge disabled">CORRUPT</span>' : ''}</div><div class="mod-actions"><button data-action="toggle-mod">${item.enabled ? 'Disable' : 'Enable'}</button><button class="mod-btn" data-action="export-mod" title="Export File">⇲</button><button class="mod-btn" data-action="edit-mod" title="Edit">✎</button><button class="mod-btn" data-action="delete-mod" title="Delete">✕</button></div></div>`);
			}
		},
		showLoader(text) {
			let loader = U_DOCUMENT.getElementById('dollynk-loader');
			if (!loader) {
				loader = U_DOCUMENT.createElement('div'); loader.id = 'dollynk-loader';
				loader.innerHTML = `<div class="loader-content"><div class="loader-title">DoLlynk</div><div class="loader-status"></div></div>`;
				U_DOCUMENT.documentElement.appendChild(loader);
			}
			loader.querySelector('.loader-status').textContent = text;
		},
		hideLoader: () => U_DOCUMENT.getElementById('dollynk-loader')?.remove(),
		showForm(formId) {
			this.dom.mainScreen?.classList.add('hidden');
			U_DOCUMENT.querySelectorAll('.mod-form').forEach(f => f.classList.add('hidden'));
			U_DOCUMENT.getElementById(formId)?.classList.remove('hidden');
		},
		resetForm() {
			U_DOCUMENT.querySelectorAll('.mod-form').forEach(f => f.classList.add('hidden'));
			this.dom.mainScreen?.classList.remove('hidden');
			state.editingId = null; this.dom.nameInput.value = ''; this.dom.codeInput.value = '';
			this.dom.earlyLoadCheck.checked = false; this.dom.formHeader.textContent = 'Add Mod Manually';
		},
		_getManagerTemplate() {
	...
	const Events = {
		bind(container) {
			container.addEventListener('toggle', e => {
				const g = e.target.dataset.groupName;
				if (g) {
					if (e.target.open) state.openGroups.add(g);
					else state.openGroups.delete(g);
					Storage.saveOpenGroups();
				}
			}, true);
			container.addEventListener('click', e => {
				...
				if (!action) return;
				const modId = e.target.closest('[data-mod-id]')?.dataset.modId;
			...
			UI.dom.dropzone.addEventListener('drop', e => { e.preventDefault(); UI.dom.dropzone.classList.remove('dragover'); FileHandler.stage(Array.from(e.dataTransfer.files)); });
			this.bindDragDrop(UI.dom.list);
		},
		bindDragDrop(container) {
			let dragTarget = null;
			const cleanup = () => { dragTarget?.classList.remove('drag-over'); dragTarget = null; };
			container.addEventListener('dragstart', e => {
				const t = e.target.closest('[draggable=true]');
				if (!t) { e.preventDefault(); return; }
				e.dataTransfer.setData('text/plain', t.dataset.modId ? `mod:${t.dataset.modId}` : `group:${t.dataset.groupName}`);
			});
			container.addEventListener('dragover', e => {
				e.preventDefault();
				const t = e.target.closest('[draggable=true]');
				if (t && t !== dragTarget) { cleanup(); t.classList.add('drag-over'); dragTarget = t; }
			});
			...
			container.addEventListener('drop', e => {
				...
				cleanup();
				if (!targetEl || !dragged) return;
				const target = targetEl.dataset.modId ? `mod:${targetEl.dataset.modId}` : `group:${targetEl.dataset.groupName}`;
				if (dragged !== target) ModActions.reorder(dragged, target);
			});
		},
		_handlers: {
			...
			'toggle-mod': (id) => ModActions.toggle(id),
			'edit-mod': (id) => ModActions.edit(id),
			'delete-mod': (id) => ModActions.delete(id),
			'export-mod': (id) => ModActions.exportMod(id),
			'toggle-group': (_, id) => ModActions.toggleGroup(id),
			'delete-group': (_, id) => ModActions.deleteGroup(id),
			'rename-group': (_, id) => ModActions.renameGroup(id),
			'export-group': (_, id, e) => ModActions.exportGroup(id, e.target),
			'add-to-group': (_, id) => ModActions.addToGroup(id),
			'save-manual': () => ModActions.saveManual(),
			'install-staged': () => FileHandler.installStaged(),
			'export-all': () => Storage.exportAll(),
			'import-all': () => {
				const i = U_DOCUMENT.createElement('input'); i.type = 'file'; i.accept = '.json,.json5';
				i.onchange = e => e.target.files[0] && Storage.importAll(e.target.files[0]); i.click();
			},
			...
			'reload-page': () => U_WINDOW.location.reload(),
		}
	};
	...
	const TweeProcessor = {
		inject(content, storyData, source) {
			const fragment = U_DOCUMENT.createDocumentFragment();
			// OPTIMIZATION: Build map of existing passages first
			const existingPassages = new Map();
			for (const node of storyData.children) {
				if (node.tagName === 'TW-PASSAGEDATA') existingPassages.set(node.getAttribute('name'), node);
			}
			...
			for (let i = 1; i < passages.length; i++) {
				...
				const passageData = this._parsePassage(text, source);
				if (passageData) {
					this._injectPassage(passageData, storyData, fragment, existingPassages);
					count++;
				}
			}
			if (fragment.childNodes.length > 0) storyData.appendChild(fragment);
			return count;
		},
		...
		_injectPassage({ name, tags, content }, storyData, fragment, existingMap) {
			const existing = existingMap.get(name);
			if (existing) {
				existing.textContent = content;
				if (tags) existing.setAttribute('tags', tags); else existing.removeAttribute('tags');
				existing.setAttribute('pid', `mod_${crypto.randomUUID()}`);
			} else {
				const el = U_DOCUMENT.createElement('tw-passagedata');
	...
	const PassagePatcher = {
		_escapedPattern(pattern) { return pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); },
		_trySearch(content, searchString, positionHint, tolerance) {
			if (content.substring(positionHint, positionHint + searchString.length) === searchString) return positionHint;
			const startPos = Math.max(0, positionHint - tolerance);
			const endPos = Math.min(content.length, positionHint + searchString.length + tolerance);
			const subContent = content.substring(startPos, endPos);
			const match = new RegExp(this._escapedPattern(searchString)).exec(subContent);
			return match ? startPos + match.index : -1;
		},
		apply(patch, passageMap) {
			const results = { success: 0, failed: 0 };
			const targets = [];
			const isLegacyRegex = patch.method === 'regex';
			const isExplicitRegex = patch.name.length > 2 && patch.name.startsWith('/') && patch.name.endsWith('/');

			if (isLegacyRegex || isExplicitRegex) {
				try {
					let pattern = patch.name;
					if (isExplicitRegex) pattern = pattern.slice(1, -1);
					const nameRegex = new RegExp(pattern);
					// OPTIMIZATION: Iterating map keys is faster than DOM queries
					for (const name of passageMap.keys()) {
						if (nameRegex.test(name)) targets.push(passageMap.get(name));
					}
				} catch (e) { console.warn('[⚠️ DoLlynk Patch] Invalid passage name regex:', patch.name); }
			} else {
				const el = passageMap.get(patch.name);
				if (el) targets.push(el);
			}

			if (!targets.length) return { success: 0, failed: 1 };

			// OPTIMIZATION: Compile regex once if possible
			let findRegex = null;
			if ((patch.isRegex || patch.method === 'regex') && !patch.positionHint) {
				findRegex = new RegExp(patch.find, 'g');
			}

			for (const el of targets) {
				let content = el.textContent;
				if (patch.method === 'replace') {
					el.textContent = patch.replace; results.success++; continue;
				}
				if (patch.positionHint !== undefined) {
					const pos = this._trySearch(content, patch.find, patch.positionHint, patch.tolerance || 250);
					if (pos > -1) {
						el.textContent = content.substring(0, pos) + patch.replace + content.substring(pos + patch.find.length);
						results.success++;
					} else { results.failed++; }
				} else {
					if (findRegex) {
	...
	const ModProcessor = {
		async applyAll(storyData) {
			if (U_WINDOW.DoLlynk.API.preprocessors?.length > 0) {
				console.log(`[🦚 DoLlynk] Running ${U_WINDOW.DoLlynk.API.preprocessors.length} API pre-processors...`);
				for (const processor of U_WINDOW.DoLlynk.API.preprocessors) {
					try { await processor(state); } catch (e) { console.error('[❌ DoLlynk] Pre-processor failed:', e); }
				}
			}
			const allMods = [...systemMod, ...state.mods.filter(m => m.enabled)];
			...
			UI.showLoader('Phase 1: Twee Injection');
			let injectedPassageCount = 0;
			const fragment = U_DOCUMENT.createDocumentFragment();
			for (const mod of allMods) {
				if (mod.type === 'twee') injectedPassageCount += TweeProcessor.inject(mod.code, storyData, `${mod.groupName || ''}/${mod.name}`);
			}
			if (injectedPassageCount > 0) console.log(`[✓ DoLlynk Twee] Injected ${injectedPassageCount} passages.`);

			// Re-sync map after injection
			for (const node of storyData.children) {
				if (node.tagName === 'TW-PASSAGEDATA') passageMap.set(node.getAttribute('name'), node);
			}

			UI.showLoader('Phase 2: Patching');
			const patchLogs = []; let totalApplied = 0; let totalFiles = 0;
			for (const mod of allMods) {
				if (mod.type === 'unified-patch') {
					const result = this._applyUnifiedPatches(mod, passageMap);
					if (result && result.log) { patchLogs.push(result.log); totalApplied += result.applied; totalFiles++; }
				}
			}
			if (patchLogs.length > 0) {
				console.groupCollapsed(`[✓ DoLlynk Patches] Summary: Applied ${totalApplied} patch(es) from ${totalFiles} file(s)`);
				patchLogs.forEach(log => console.log(log));
				console.groupEnd();
			}
			UI.showLoader('Phase 3: Scripts & Styles');
			for (const mod of allMods) {
				if (mod.type === 'js' && mod.earlyLoad) this._injectScript(mod, 'early');
				if (mod.type === 'css') this._injectStyle(mod);
			}
		},
		_applyUnifiedPatches(mod, passageMap) {
			...
			try {
				...
				for (const patch of patches) {
					if (patch.findFile) {
						const f = state.mods.find(m => m.groupName === mod.groupName && ModActions._getFileNameFromMod(m) === patch.findFile);
						if (f) patch.find = f.code;
					}
					if (patch.replaceFile) {
						const f = state.mods.find(m => m.groupName === mod.groupName && ModActions._getFileNameFromMod(m) === patch.replaceFile);
						if (f) patch.replace = f.code;
					}
					switch (patch.role) {
						case 'passage': {
							const r = PassagePatcher.apply(patch, passageMap);
							stats.pass += r.success; stats.fail += r.failed;
							if (r.failed > 0) stats.failed.push(patch.name || `find: "${String(patch.find).substring(0, 30)}..."`);
							break;
						}
						case 'style':
				...
				const appliedCount = stats.pass + stats.style;
				if (appliedCount > 0 && stats.fail === 0) return { applied: appliedCount, log: `${mod.name}: Applied ${appliedCount} item(s)` };
				if (stats.fail > 0) {
					console.warn(`[⚠️ DoLlynk Patch] ${mod.name}: Failed to apply ${stats.fail} patch(es). Applied ${appliedCount}.`);
					stats.failed.forEach(n => console.warn(`  └─ Patch: "${n}"`));
				}
		...
		_injectScript(mod, phase) {
			const s = U_DOCUMENT.createElement('script');
			s.id = `dol-mod-js-${phase}-${mod.id}`;
			s.textContent = `try{${mod.code}}catch(e){console.error('[❌ DoLlynk] Script Error in ${mod.name} (${phase}):',e)}`;
			U_DOCUMENT.head.appendChild(s);
		},
		_injectStyle(mod) {
			const s = U_DOCUMENT.createElement('style');
			s.id = `dol-mod-css-${mod.id}`;
			s.textContent = mod.code;
			U_DOCUMENT.head.appendChild(s);
		}
	};
			...
			for (const mod of patches) {
				...
				try {
					...
					for (const patch of scriptPatches) {
						let occurrences = 0;
						if (patch.positionHint !== undefined) {
							const pos = PassagePatcher._trySearch(content, patch.find, patch.positionHint, patch.tolerance || 250);
							if (pos > -1) {
								content = content.substring(0, pos) + patch.replace + content.substring(pos + patch.find.length);
								occurrences = 1;
							}
						} else {
							...
							} catch (e) { console.error(`[❌ DoLlynk] Invalid Regex in "${mod.name}": `, patch.find); continue; }
							if (occurrences > 5) {
								console.warn(`[🛡️ DoLlynk] Skipped generic script patch in "${mod.name}" (over 5 matches for: ${patch.find})`); continue;
							}
							if (occurrences > 0) content = isRegex ? content.replace(new RegExp(patch.find, 'g'), patch.replace) : content.replaceAll(patch.find, patch.replace);
						}
						if (occurrences > 0) { modPatched++; patchedTotal++; }
					}
					if (modPatched > 0) log.push(`${mod.name}: ${modPatched}`);
				} catch (e) { console.error(`[❌ DoLlynk] Error parsing script patches in ${mod.name}:`, e); }
			}
			if (patchedTotal > 0) {
				try {
					new Function(content); twineScript.textContent = content;
					console.log(`[✓ DoLlynk Scripting] Applied ${patchedTotal} early-load patches from [${log.join(', ')}]`);
				} catch (err) { console.error(`[❌ DoLlynk] FATAL SCRIPT ERROR post-patching. Game may not load.`, err); }
			}
		}
	};
	...
	async function initializeModSystem(sugarcubeScript, parent) {
		...
		try {
			...
			if (twineScript) PlatformPatcher.applyTwineScriptPatches(twineScript);
			else console.warn('[⚠️ DoLlynk] Could not find #twine-user-script to patch.');

			parent.appendChild(sugarcubeScript);
			Hooks._patchSugarcube();
			UI.hideLoader();
			const $ = U_WINDOW.jQuery || U_WINDOW.$;
			if ($) $(U_DOCUMENT).one(':storyready', () => {
				console.log('[🦚 DoLlynk] Story Ready - Injecting Runtime Scripts');
				ModProcessor.injectRuntime();
			});
			else setTimeout(() => ModProcessor.injectRuntime(), 1000);
		} catch (err) {
	...
	U_DOCUMENT.addEventListener('keydown', (e) => {
		if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'm') {
			e.preventDefault(); e.stopPropagation();
			try { UI.forceInjectManager(); } catch (err) { console.error('[❌ DoLlynk] Failsafe UI Error:', err); }
		}
	}, true);
	U_WINDOW.DoLlynk = {
		state, UI, ModActions, FileHandler, Storage, Events, Assets, ENV,
		API: {
			subscribe: Hooks.subscribe.bind(Hooks),
			getMod: (name) => state.mods.find(m => m.name === name || m.id === name),
			getAllMods: () => [...state.mods],
			getPassage: (name) => U_WINDOW.Story?.get(name),
			preprocessors: []
		}
	};
})();
